...
const ProposalStep: React.FC<ProposalStepProps> = ({ open, onClose, prospectName }) => {
      ...
      <SheetContent side="right" className="w-full md:w-[600px] lg:w-[800px] max-w-full">
        <div className="bg-background">
          <div className="flex items-center justify-between border-b border-border sticky top-0 bg-background z-10 px-2 sm:px-6 py-4">
            <h2 className="text-xl font-bold flex items-center gap-2">Proposal for {prospectName}</h2>
          ...
          <div className="py-4 border-b border-border flex-1 overflow-y-auto px-2 sm:px-6">
            <div className="mb-2 font-semibold">Proposal Files</div>
            {files.length === 0 ? (
              <div className="text-sm text-muted-foreground mb-4">No proposal files uploaded yet.</div>
            ) : (
              <ul className="mb-4">
                {files.map((f, i) => (
            ...
            <div className="flex items-center gap-2 mb-4">
              <input type="file" multiple ref={fileInputRef} onChange={handleFileChange} className="hidden" id="proposal-upload" />
              <button type="button" className="px-3 py-2 rounded bg-muted text-foreground hover:bg-muted/80 flex items-center gap-2" onClick={() => fileInputRef.current?.click()} disabled={uploading}>
                <Upload className="w-5 h-5" /> Upload Proposal
              </button>
            </div>
            <div className="mb-4">
              <label className="font-medium mb-1 block">Proposal Description</label>
              <textarea className="border rounded px-2 py-2 text-sm focus:ring-2 focus:ring-primary min-h-[80px] w-full mb-2" placeholder="Describe the proposal..." value={desc} onChange={e => setDesc(e.target.value)} />
